...
let speedMoveBallTop = getRandomIntInclusive(9, 21);
let speedMoveBallLeft = 45;
...
/* ________Фиксатор отражения или пропуска мяча слева______________ */
    ...
    if (Number(ballPositionLeft.toFixed(1)) <= Number(racketWidth.toFixed(1))) {
        ...
        if (Number(ballPositionTop.toFixed(1)) > Number((playerPositionTop - ballWidth * 2).toFixed(1)) && Number(ballPositionTop.toFixed(1)) < Number((playerPositionTop + racketHeight + ballWidth / 2).toFixed(1))) {
            console.log("Позиция игрока: " + playerPositionTop + " Позиция мяча топ: " + ballPositionTop + ' Позиция мяча лефт ' + ballPositionLeft);
...
/* ________Фиксатор отражения или пропуска мяча справа______________ */   
        ...
        if (Math.floor(ballPositionTop) > Math.ceil(enemyPositionTop - ballWidth / 2) && Math.floor(ballPositionTop) < Math.ceil(enemyPositionTop + racketHeight + ballWidth / 2)) {
            ...
            console.log("Позиция игрока: " + playerPositionTop + " Позиция мяча: " + ballPositionTop + ' Позиция врага: ' + enemyPositionTop);
            return 'enemyFall'
        }
    }
    else {
...
function hitBallChangeDirection(racket, ballPositionTop) {
    switch (getPlaceContactRacketWithBall(racket, ballPositionTop)) {
        case 0:
            setDirectionHit(21, -1);
            break
        case 1:
            setDirectionHit(15, -1);
            break
        case 2:
            setDirectionHit(9, -1);
            break
...
function ston1 () {
    ...
    audio.src = 'sound/ston1.wav'; // Указываем путь к звуку "клика"
    audio.autoplay = true; // Автоматически запускаем
  }

function ston2 () {
    let audio = new Audio(); // Создаём новый элемент Audio
    audio.src = 'sound/ston2.wav'; // Указываем путь к звуку "клика"
    audio.autoplay = true; // Автоматически запускаем
}

function ston3 () {
    let audio = new Audio(); // Создаём новый элемент Audio
    audio.src = 'sound/ston3.wav'; // Указываем путь к звуку "клика"
    audio.autoplay = true; // Автоматически запускаем
}

function ston4 () {
    let audio = new Audio(); // Создаём новый элемент Audio
    audio.src = 'sound/ston4.mp3'; // Указываем путь к звуку "клика"
    audio.autoplay = true; // Автоматически запускаем
}
...
function ballMoveAnimation () {
    ...
    switch (hitTheBall(ballPositionTop, ballPositionLeft)) {
        case 'playerHit':
            ...
            return setTimeout (ballMoveAnimation, 9, ballStepSizeTop, ballStepSizeLeft)
        case 'enemyHit':
            changeDirectionHit ()
...
function gameStart (evt) {
    ...
    if (evt.code == "Space") {
        disabledWindowsMenu (); 
        gameOnOff = setTimeout (ballMoveAnimation, 9, ballStepSizeTop, ballStepSizeLeft);
